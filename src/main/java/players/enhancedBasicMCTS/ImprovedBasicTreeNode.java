...
class ImprovedBasicTreeNode {
    ...
    private EnhancedBasicMCTS player;
    private Random rnd;
    ...
    // IMPROVEMENT: Track best and worst values seen (for early termination)
    private double maxValueSeen = -Double.MAX_VALUE;
    private double minValueSeen = Double.MAX_VALUE;

    protected ImprovedBasicTreeNode(EnhancedBasicMCTS player, ImprovedBasicTreeNode parent,
    ...
    void mctsSearch() {
        ...
        while (!stop) {
            ...
            // IMPROVEMENT: Early termination if one action dominates
            if (numIters > 100 && shouldTerminateEarly()) {
                break;
            }
    ...
    /**
     * IMPROVEMENT: Check if one action is clearly dominant
     */
    private boolean shouldTerminateEarly() {
        if (children.isEmpty() || nVisits < 200) {
            return false;
        }

        // Find action with most visits
        int maxVisits = 0;
        int secondMaxVisits = 0;

        for (ImprovedBasicTreeNode child : children.values()) {
            if (child != null) {
                if (child.nVisits > maxVisits) {
                    secondMaxVisits = maxVisits;
                    maxVisits = child.nVisits;
                } else if (child.nVisits > secondMaxVisits) {
                    secondMaxVisits = child.nVisits;
                }
            }
        }

        // If one action has 3x more visits than second best, it's clearly dominant
        return maxVisits > 3 * secondMaxVisits && maxVisits > 50;
    }
    ...
    private AbstractAction ucb() {
        ...
        for (AbstractAction action : children.keySet()) {
            ...
            // IMPROVEMENT: Slightly boost exploration for unvisited nodes
            double explorationTerm = params.K * Math.sqrt(Math.log(this.nVisits + 1) / (child.nVisits + params.epsilon));

            // IMPROVEMENT: Add first-play urgency bonus
            if (child.nVisits < 3) {
                explorationTerm += 0.5;  // Small bonus for very new nodes
            }
     ...
     * IMPROVEMENT: Smarter rollout with occasional heuristic guidance
     *
    ...
    private double rollOut() {
        ...
        if (player.getParameters().rolloutLength > 0) {
            while (!finishRollout(rolloutState, rolloutDepth)) {
                ...
                // IMPROVEMENT: 30% of the time, pick action using heuristic instead of random
                if (rnd.nextDouble() < 0.3 && availableActions.size() > 1) {
                    next = selectByHeuristic(rolloutState, availableActions);
                } else {
        ...
        // IMPROVEMENT: Track min/max values
        if (value > root.maxValueSeen) root.maxValueSeen = value;
        if (value < root.minValueSeen) root.minValueSeen = value;

        return value;
    }

    /**
     * IMPROVEMENT: Select action by evaluating with heuristic
     */
    private AbstractAction selectByHeuristic(AbstractGameState state, List<AbstractAction> actions) {
        AbstractAction bestAction = null;
        double bestValue = -Double.MAX_VALUE;
        ...
        for (AbstractAction action : actions) {
            ...
            double value = player.getParameters().getStateHeuristic()
                    .evaluateState(testState, player.getPlayerID());
        ...
        return bestAction != null ? bestAction : actions.get(rnd.nextInt(actions.size()));
    }
     ...
     * IMPROVEMENT: Better tiebreaking using average value
     *
    ...
    AbstractAction bestAction() {
        ...
        for (AbstractAction action : children.keySet()) {
            if (children.get(action) != null) {
                ...
                // IMPROVEMENT: Primary criterion is visits, but add value as tiebreaker
                double childValue = node.nVisits;

                // Add 1% of average value as tiebreaker
                if (node.nVisits > 0) {
                    double avgValue = node.totValue / node.nVisits;
                    childValue += 0.01 * avgValue;
                }

                // Apply small noise to break ties randomly
                childValue = noise(childValue, player.getParameters().epsilon, player.getRnd().nextDouble());

                // Save best value (highest visit count + value tiebreaker)
