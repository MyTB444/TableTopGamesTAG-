...
import players.improvedBasisMCTS.ImprovedBasicMCTSPlayer;
...
public class ImprovedBasicMCTSParams extends PlayerParameters {
    ...
    public double K = 0.7;  // Lower for more focused exploitation
    ...
    public double raveK = 300;  // Controls how quickly RAVE influence decreases

    // Progressive widening parameters (for large action spaces)
    public boolean useProgressiveWidening = true;
    public double progressiveWideningAlpha = 0.5;  // Controls growth rate
    public double progressiveWideningC = 2.0;      // Base constant

    // Enhanced selection parameters
    public boolean useUCB1Tuned = false;  // UCB1-Tuned uses variance
    ...
    public ImprovedBasicMCTSParams() {
        // Standard parameters
        addTunableParameter("K", 0.7, Arrays.asList(0.0, 0.3, 0.5, 0.7, 1.0, Math.sqrt(2), 2.0));
        addTunableParameter("rolloutLength", 0, Arrays.asList(0, 3, 5, 10, 20));
        addTunableParameter("maxTreeDepth", 20, Arrays.asList(5, 10, 15, 20, 30, 50));
        addTunableParameter("epsilon", 1e-6);
        addTunableParameter("heuristic", (IStateHeuristic) AbstractGameState::getHeuristicScore);

        // RAVE parameters
        addTunableParameter("useRAVE", true);
        addTunableParameter("raveK", 300.0, Arrays.asList(100.0, 300.0, 500.0, 1000.0));

        // Progressive widening
        addTunableParameter("useProgressiveWidening", true);
        addTunableParameter("progressiveWideningAlpha", 0.5, Arrays.asList(0.25, 0.5, 0.75));
        addTunableParameter("progressiveWideningC", 2.0, Arrays.asList(1.0, 2.0, 3.0, 5.0));

        // Selection enhancements
        addTunableParameter("useUCB1Tuned", false);
